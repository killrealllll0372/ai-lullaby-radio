...
The implementation is intentionally frontend-only. The app plays bundled MP3 files, reads static metadata, and keeps state inside the browser. No backend is required for the current feature set.

## Stack

- Vite
- TypeScript
- Two browser `Audio` elements for overlap transitions
...
- Parent-friendly Russian UI without visible track titles.
...
- Fade-out before pause and timer stop.
- Crossfade between tracks, including manual next-track changes.
...
The MVP does not calculate LUFS or perform offline normalization. As a practical first protection, playback volume is capped and changed gradually. Track starts fade in from silence, pause/timer shutdown fades out before stopping, and track changes overlap with a short crossfade.
